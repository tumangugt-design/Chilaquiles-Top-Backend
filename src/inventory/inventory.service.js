...
export const recalculatePortionCost = async (name) => {
  const normalized = normalizeName(name)
  const invItem = await Inventory.findOne({ name: normalized })
  const portion = await Portion.findOne({ name: normalized })

  if (!invItem || !portion) {
    const error = new Error('Producto o porción no encontrada')
  ...
  if (!(qty > 0) || !(totalPrice > 0) || !unit) {
    const error = new Error('No hay compras registradas para calcular el costo de este producto.')
    error.statusCode = 400
    throw error
  }
  ...
  const newPrice = Math.round(unitPrice * portionInBaseUnit * 100) / 100

  portion.price = newPrice
  await portion.save()
  invItem.lastPrice = newPrice
  await invItem.save()
