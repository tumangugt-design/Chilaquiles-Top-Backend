...
export const prepareBotContext = (customerName, orderHistory, operatingHours, conversationSummary) => {
  ...
  const pricingInfo = `1 plato Q${ORDER_PRICING[1]}, 2 platos Q${ORDER_PRICING[2]}, 3 platos Q${ORDER_PRICING[3]}.`;
  ...
  if (orderHistory && orderHistory.length > 0) {
    historyContext = 'Historial de compras previas del cliente:\n' + orderHistory.map(order => {
      ...
      if (order.status === 'en_camino' && order.whatsappMessages?.orderOnTheWay?.sentAt) {
        const timeStr = new Date(order.whatsappMessages.orderOnTheWay.sentAt).toLocaleTimeString('es-GT', {timeZone: 'America/Guatemala', hour: '2-digit', minute:'2-digit', hour12: false});
        timeInfo = ` (Enviado a las ${timeStr}, tiempo estimado 30 a 45 min)`;
      } else if (order.status === 'recibido' && order.createdAt) {
        const timeStr = new Date(order.createdAt).toLocaleTimeString('es-GT', {timeZone: 'America/Guatemala', hour: '2-digit', minute:'2-digit', hour12: false});
